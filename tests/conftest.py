...
from repositories.store import CategoryRepository, ManufacturerRepository, ManufacturerManagerRepository, \
    CustomerRepository, CustomerManagerRepository, StaffManagerRepository, ProductRepository, OrderRepository, \
    ReviewRepository
...
async def prepare_database():
    ...
    async with async_session() as session:
        ...
        permission = PermissionCreate(title='moderation')
        user_permission = PermissionUserLink(permission_id=1, user_id=1)
        ...
        await ProductRepository(session).add_one(product1)
        await ProductRepository(session).add_one(product2)
        await OrderRepository(session).add_one(order)
        await ReviewRepository(session).add_one(review)
        await UserRepository(session).add_one(user)

        session.add(Permission.from_orm(permission))
        await session.commit()
        session.add(PermissionUserLink.from_orm(user_permission))
        await session.commit()
